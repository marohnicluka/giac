...
void graphe::make_random_flow_network(const vecteur &V,int capacity) {
    ...
    ivector &largest_face=faces[k];
    k=rand_integer(n);
    int source=largest_face[k],sink=largest_face[(k+n/2)%n];
    int c=0,i,j;
    ivector fromsrc(G.node(source).neighbors().size());
    for (k=fromsrc.size();k-->1;) {
        c+=(fromsrc[k]=1+rand_integer((capacity-c)/(k+1)));
    }
    fromsrc.front()=capacity-c;
    ipairs E;
    G.get_edges_as_pairs(E,false);
    vector<bool> visited(E.size(),true);
    matrice A=*_matrix(makesequence(G.node_count(),G.edge_count(),0),ctx)._VECTptr;
    ivector source_edges;
    c=0;
    G.dfs(source,true,true,NULL,-1,false,sink);
    // set directions to the edges visited by dfs
    for (ipairs_iter it=E.begin();it!=E.end();++it) {
        i=it->first;
        j=it->second;
        k=it-E.begin();
        vertex &v=G.node(i),&w=G.node(j);
        if (i==source || j==source)
            source_edges.push_back(k);
        else if (i==sink || j==source) {
            A[i]._VECTptr->at(k)=1;
            A[j]._VECTptr->at(k)=-1;
        } else if (j==sink || i==source) {
            A[i]._VECTptr->at(k)=-1;
            A[j]._VECTptr->at(k)=1;
        } else if (i==w.ancestor()) {
            A[i]._VECTptr->at(k)=-1;
            A[j]._VECTptr->at(k)=1;
    ...
    // set directions to all other edges
    for (ipairs_iter it=E.begin();it!=E.end();++it) {
        i=it->first;
        j=it->second;
        k=it-E.begin();
        ...
        if (!v.is_leaf() && !w.is_leaf()) {
            int r=1-2*rand_integer(2);
            A[i]._VECTptr->at(k)=r;
            A[j]._VECTptr->at(k)=-r;
        } else {
            // exactly one node is a leaf
            assert(v.is_leaf()!=w.is_leaf());
            A[i]._VECTptr->at(k)=v.is_leaf()?-1:1;
            A[j]._VECTptr->at(k)=w.is_leaf()?-1:1;
            v.set_is_leaf(false);
            w.set_is_leaf(false);
        }
    }
    vecteur b(A.size(),0);
    for (ivector_iter it=source_edges.begin();it!=source_edges.end();++it) {
        vecteur row(A.front()._VECTptr->size(),0);
        row[*it]=1;
        A.push_back(row);
        b.push_back(fromsrc[it-source_edges.begin()]);
    }
    // compute arc capacities
    matrice Areduced(A);
    Areduced.erase(Areduced.begin()+source);
    Areduced.erase(Areduced.begin()+sink);
    b.erase(b.begin()+source);
    b.erase(b.begin()+sink);
    gen opt=_lpsolve(makesequence(vecteur(0),makevecteur(vecteur(0),vecteur(0),Areduced,b),
                                  symbolic(at_equal,makesequence(at_assume,_LP_NONNEGINT))),ctx);
    assert(opt.type==_VECT && opt._VECTptr->size()==2 &&
           opt._VECTptr->back().type==_VECT && !opt._VECTptr->back()._VECTptr->empty());
    vecteur &arc_weights=*opt._VECTptr->back()._VECTptr;
    ...
    for (ipairs_iter it=E.begin();it!=E.end();++it) {
        ...
        k=it-E.begin();
        dir=is_one(A[j][k]);
        add_edge(dir?i:j,dir?j:i,arc_weights[k]);
    }
}
