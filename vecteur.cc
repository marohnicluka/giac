...
#define CLOCKS_PER_SEC 1e6
  ...
  */
  ...
  matrice csv2gen(istream & i,char sep,char nl,char decsep,char eof,GIAC_CONTEXT){
          ...
          try {
            if (s[0]=='=' || s[0]=='-'){
              ...
              if (s[0]==decsep ||(s[0]>='0' && s[0]<='9')){
                ...
                line.push_back(string2gen(s,s[0]=='"'));
                // addition by L.Marohnić: trim whitespace from string elements
                // (this resolves e.g. unwanted characters appearing in the
                // output after importing a file with \r\n line separator)
                if (line.back().type==_STRNG) {
                  string &cs=*line.back()._STRNGptr;
                  for (int pass=1;pass<=2;++pass) {
                    while (!cs.empty() && (cs[0]==32 || (cs[0]>8 && cs[0]<14)))
                      cs=cs.substr(1);
    ...
    // now make a matrix from res (change by L.Marohnić: skip empty lines)
    for (unsigned j=nrows;j-->0;){
      vecteur &rj=*res[j]._VECTptr;
      if (rj.size()==1 && rj.front().type==_STRNG && rj.front()._STRNGptr->empty())
        res.erase(res.begin()+j); // that's an empty line
      else rj.resize(ncols,0);
    }
    return res;
  }
